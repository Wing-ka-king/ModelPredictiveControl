...
class MPC(object):
    ...
    def __init__(self, model, dynamics,
                 horizon=None, Q=None, P=None, R=None,
                 ulb=None, uub=None, xlb=None, xub=None, terminal_constraint=None,
                 solver_opts=None, integrator=False
                ):
        #print(terminal_constraint)        
        ...
        if integrator is False:
            self.Nx, self.Nu = len(model.x_d), len(model.u_eq)
        else:
            self.Nx, self.Nu = len(model.x_d_ag), len(model.u_eq)
        ...
        # Decision variable boundries  (Xlb <= X <= Xub)
        ...
        con_ineq_lb = []
        con_ineq_ub = []
        con_eq.append(opt_var['x', 0] - x0)  ####################################### why equality constraint like this ?
        ...
        for t in range(self.Nt):
            ...
            # Input constraints      ####################################### how it works
            if uub is not None:
                con_ineq.append(u_t)
    ...
    def set_cost_functions(self):
        """
        Set cost functions for the MPC.
        """
    ...
    def solve_mpc(self, x0, u0=None):
        ...
        #print("self.optvar_init['x', 0] = ",self.optvar_init['x', 0])
    ...
    def mpc_controller(self, x0):
        """
        Obtain a control input using MPC.

        :param x0: current state.
        :type x0: ca.DM or np.array
    ...
    def set_reference(self, x_sp):
        ...
        :param x_sp: setpoint for the MPC
        :type x_sp: ca.DM or np.array
        """
        self.x_sp = x_sp

    def constant_control(self, x0):

        return self.u_const

    def set_constant_control(self, u):

        self.u_const = np.array(u)
