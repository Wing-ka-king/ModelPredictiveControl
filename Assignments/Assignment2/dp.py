...
class Gridder(object):
    ...
    def set_cost_functions(self):
        ...
        if self.use_nonlinear_controller is False:
            ...
            b0 = self.model.b0
            self.Jstage = ca.Function('Jstage', [x, Q, u, R], \
                                      [x.T @ Q @ x + R * ( 1/ca.cos(x[0]) * ( a0/b0 * ( ca.sin(x[0]) - x[0] ) + u ) )**2] ) 
            ...
            for i in range(len(self.x1)):
                for j in range(len(self.x2)):
                    ...
                    if self.use_nonlinear_controller is False:
                        ...
                        a0 = self.model.a0
                        b0 = self.model.b0
                        self.U[i,j,n] = ( 1/ca.cos(x[0]) * ( a0/b0 * ( ca.sin(x[0]) - x[0] ) + u ) )
...
class BiggerGridder(object):
    ...
    def create_grid(self):
        ...
        eps = 0.1
        self.x1 = np.linspace(-2+eps, 10, 10)
        self.x2 = np.linspace(-5, 5, 5 )

        eps = np.pi/10
        self.x3 = np.linspace(-np.pi/2+eps, np.pi/2-eps, 10 )
    ...
    def set_cost_functions(self):
        ...
        self.Jstage = ca.Function('Jstage', [x, Q, u, R], \
                                      [(x-self.xd).T @ Q @ (x-self.xd) + u.T @ R @ u] )

        self.Jtogo = ca.Function('Jtogo', [x], \
                                  [(x-self.xd).T @ self.Q @ (x-self.xd)] )
